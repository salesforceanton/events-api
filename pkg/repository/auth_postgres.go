...
func (s *AuthPostgres) CreateUser(user domain.User) (int, error) {
	var result int
...
func (s *AuthPostgres) GetUser(username, password string) (domain.User, error) {
	var result domain.User

	query := fmt.Sprintf("SELECT id FROM %s WHERE username=$1 AND password_hash=$2", USERS_TABLE)
	err := s.db.Get(&result, query, username, password)
